...
\begin{enumerate}
    ...
    \item \textbf{An exploratory analysis pipeline} connecting blind preference
          ...
          diagnostics.
    \item \changed{\textbf{A computational preference judge} ($J_{final}$), an interpretable
...
computational preference judge (RQ2). Chapter~\ref{ch:mariodpo} uses these
...
\changed{The PCG Arena platform hosts 14 generators spanning all major paradigm
...
\changed{An ERA characterisation of the PCG Arena generators themselves is
...
\midrule
\textbf{PCG Arena (ours)}   & \textbf{2026} & \changed{\textbf{14 generators (all paradigms)}} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\bottomrule
\end{tabular}
...
\changed{RLHF trains a policy to maximise a reward signal derived from human
preferences rather than a hand-designed reward function.
...
used to train InstructGPT, demonstrating that alignment quality matters more
than model scale. The lesson transfers directly to PCG: just as ROUGE fails to
...
\KOT{co, kto, z czym w ogóle o co chodzi>}
\KOT{ke? odnosisz sie do czegoś co wie ktokolwiek kto czyta tą pracę a nie rozmawia z tym samym LLMem?}
...
The vote outcomes are balanced between the two screen sides (418 left wins and 398 right wins), with 165 ties and 19 skips, indicating no systematic side bias; moreover, the assignment of levels to the left or right side was randomised, further reducing the risk of positional bias. The median play duration is 8.0 seconds, \changed{though a small number of inactive-tab outliers are present}.
...
\changed{Aggregating individual votes into the single ranking of
Section~\ref{sec:eda-rankings} is only meaningful if the underlying player
population is reasonably consistent; this section examines who the voters are
and whether they agree.} The anonymous-player data are sparse, and because players are identified only by browser/player IDs we know nothing about who they are---their age, background, or gaming experience are all unknown. The data are nonetheless useful for understanding engagement and possible preference heterogeneity. Figure~\ref{fig:eda-engagement} shows votes over time, votes per anonymous player ID, and cumulative vote contribution.
...
\caption{\changed{Preference heatmap of anonymous player IDs against generators.
...
placed next to one another. Columns follow the global generator ranking. The
...
\changed{This agreement is what justifies the two uses the rest of the thesis
makes of the votes: combining them into a single preference ranking
(Section~\ref{sec:eda-rankings}), and using them as a training signal for
preference-aligned generation (Chapter~\ref{ch:mariodpo}).}
...
and how similar its columns are to one another. \changed{Computing the same
numbers for every generator makes it possible to compare generators directly
and to} check whether any of these structural properties line up with the player
...
\changed{For the judge function developed in
Section~\ref{sec:dpo-judge}, these observations identify the structural terms
worth encoding: reward density and unique-column ratio associate positively
with preference, NCD distance from the original corpus negatively, and
gap-heavy structure emerges as the dominant structural failure mode---together
motivating the judge's style and gap-penalty terms.}
...
\changed{For the judge function (Section~\ref{sec:dpo-judge}), the behavioural
findings supply the dynamic terms: occupancy entropy and verticality associate
positively with preference, concentrated death locations negatively, and
hesitation captures poor flow---motivating the judge's verticality and flow
terms.}
...
\changed{These patterns corroborate the design of the judge function
(Section~\ref{sec:dpo-judge}): the concentration of \texttt{impossible} and
\texttt{too\_hard} on losing sides supports a strong frustration (gap)
penalty, while the alignment of \texttt{fun} and \texttt{creative} with
winning sides validates that votes track enjoyment rather than noise.}

% ===================================================================
% MOVED(agent): from old §5.3 (M3), verbatim except where wrapped in \changed{}
\section[A Computational Judge of Level Preference]{\changed{A Computational Judge of Level Preference}}
\label{sec:dpo-judge}

\changed{The preceding sections identified consistent correlates of preference:
similarity to the original corpus and reward density
(Section~\ref{sec:eda-static-expressive-range}), verticality and flow
(Section~\ref{sec:eda-trajectory-fingerprints}), and the frustration modes
surfaced by tags (Section~\ref{sec:eda-tag-semantics}). This section
operationalises these findings into a single computable score, $J_{final}$,
that predicts human preference from structural and telemetry features alone.
The judge serves two purposes: it tests whether the observed correlates
generalise into a predictive model, and---as Chapter~\ref{ch:mariodpo}
shows---it provides the scalable supervision signal needed to train a
preference-aligned generator. The experiments below were run on the January
training snapshot (Table~\ref{tab:eda-timeline}), i.e., the data available at
generator-training time; the May analyses above corroborate the same
directional findings.}

\changed{The judge combines a small number of interpretable terms---style
distance, gap penalty, verticality, and flow---whose weights were chosen from
the preference patterns observed in the PCG Arena data.} \changed{Three findings from the preceding sections
(Sections~\ref{sec:eda-static-expressive-range}--\ref{sec:eda-tag-semantics})
guide its design:}
...
input only the raw tile grid of a candidate level and returns a scalar. \changed{This
is necessary because the levels it has to score are freshly generated and have
never been played, so no gameplay traces or player tags exist for them.} The trajectory
...
\changed{Section~\ref{sec:eda-trajectory-fingerprints} showed that} higher death counts are generally associated
...
\changed{(Chapter~\ref{ch:mariodpo})} can still increase the probability of novel levels, provided that their
...
\begin{table}[H]
\centering
\caption{Weights used by the \changed{judge function, with the experimental and
exploratory evidence motivating each term}.}
\label{tab:dpo-judge-weights}
\footnotesize
\begin{tabular}{@{} l c p{3.4cm} p{5.6cm} @{}}
\toprule
\textbf{Term} & \textbf{Weight} & \textbf{Rationale} & \changed{\textbf{Evidence}} \\
\midrule
$w_{style}$ & 0.32 & Original-centroid similarity, Experiment D & \changed{Exp.~D (\S\ref{subsec:dpo-judge-style}); NCD distance to \texttt{original} corpus (\S\ref{sec:eda-static-expressive-range})} \\
$w_{vert}$  & 0.26 & Verticality reward, Experiment A & \changed{Exp.~A (\S\ref{subsec:dpo-judge-vert}); occupancy entropy and verticality (\S\ref{sec:eda-trajectory-fingerprints})} \\
$w_{flow}$  & 0.10 & Penalise hesitation and poor flow & \changed{hesitation analysis (\S\ref{subsec:dpo-judge-hazard}); trajectory flow patterns (\S\ref{sec:eda-trajectory-fingerprints})} \\
$w_{gap}$   & 0.50 & Strong penalty for gap-heavy/pattern-like failure modes & \changed{Exp.~B (\S\ref{subsec:dpo-judge-hazard}); gap-dominated expressive range (\S\ref{sec:eda-static-expressive-range}); \texttt{impossible}/\texttt{too\_hard} tags (\S\ref{sec:eda-tag-semantics})} \\
$w_{comp}$  & 0.20 & Reward observed completion without making ease the sole target & \changed{progress and death-concentration findings (\S\ref{sec:eda-trajectory-fingerprints})} \\
\bottomrule
\end{tabular}
\end{table}
...
\changed{Chapter~\ref{ch:mariodpo} uses $J_{final}$ to synthesise preference
pairs at scale, with human votes retained at higher weight.}

% ===================================================================
\section[Summary: A Reliable but Sparse Signal]{\changed{Summary: A Reliable but Sparse Signal}}
...
\begin{enumerate}[nosep]
    ...
    \item \changed{\textbf{Judge function}: The preference correlates above can be combined into a single computational judge, $J_{final}$, whose generator-level scores correlate with human win rates at $r_s = 0.736$ (Section~\ref{sec:dpo-judge}).}
\end{enumerate}

\changed{Two properties of this signal matter for what follows. It is
\emph{reliable}: anonymous players largely agree on which generators are good
(Section~\ref{sec:eda-user-heterogeneity}), so the aggregate ranking is not an
artefact of individual taste. It is also \emph{sparse} at training time: the
...
directly. Section~\ref{sec:dpo-judge} supplies the missing piece---a validated
computational proxy for human preference. The next chapter closes the loop,
using the human votes and the judge together to train a preference-aligned
generator.}
...
\section[Motivation: From Sparse Votes to a Training Signal]{\changed{Motivation: From Sparse Votes to a Training Signal}}
\label{sec:dpo-motivation}

\changed{The previous chapter delivered three assets. First, blind pairwise
votes produce a reliable ranking signal: anonymous players largely agree on
which generators are good (Section~\ref{sec:eda-user-heterogeneity}). Second,
the analysis identified measurable correlates of preference---style
similarity, reward density, verticality, flow, and the avoidance of gap-heavy
frustration
(Sections~\ref{sec:eda-static-expressive-range}--\ref{sec:eda-tag-semantics}).
Third, these correlates were distilled into a validated computational judge,
$J_{final}$, which predicts generator-level human win rates at $r_s = 0.736$
(Section~\ref{sec:dpo-judge}). What the arena does \emph{not} deliver is
scale: human votes are the most reliable supervision signal, but they are too
sparse to train a sequence model directly. The January 2026 training snapshot
contains only 473 usable non-tie votes---orders of magnitude below the
preference datasets used to align language models (e.g., the 64K comparisons
of \citet{stiennon2020learning}) and far too few to fine-tune a level
generator on preference alone.}

\changed{MarioDPO therefore combines three ingredients. A strong supervised
prior trained on the original Nintendo levels ensures the model already speaks
valid Mario syntax before any preference optimisation begins. The 473 human
pairs, upweighted $10\times$, anchor training to genuine human judgement. And
the judge function from Section~\ref{sec:dpo-judge} labels 2,606 additional
synthetic pairs, expanding the dataset to a trainable scale without abandoning
the human preference objective.}
...
\begin{enumerate}[nosep]
  ...
  \item \changed{a preference dataset combining the human PCG Arena votes with
        synthetic pairs labelled by the judge function developed in
        Section~\ref{sec:dpo-judge}; and}
  \item a DPO fine-tuning stage \changed{that aligns the base generator with
...
training signal, while \changed{pairs labelled by the judge function
(Section~\ref{sec:dpo-judge})} primarily broaden coverage.}
\label{fig:dpo-pipeline}
\end{figure}

\section[Level Representation and Base Model]{\changed{Level Representation and Base Model}}
...
% AC(agent): M5 — "Training Pipeline" wrapper section deleted; the SFT
% subsection below is absorbed into "Level Representation and Base Model";
% the label sec:dpo-training is reattached to the promoted "Preference Dataset
% Construction" section so old refs resolve.
\subsection{Supervised Fine-Tuning (SFT)}
\label{subsec:dpo-sft}

\changed{Having fixed the level representation, we now train the base policy
that the DPO stage will later fine-tune.} The base policy $\pi_{ref}$ is a GPT-2-style autoregressive decoder trained on
...
\changed{The DPO dataset contains two sources of pairwise supervision: direct
human votes from PCG Arena and synthetic pairs scored by the judge function
developed in Section~\ref{sec:dpo-judge}.}
...
\section[DPO Fine-Tuning and Inference-Time Filtering]{\changed{DPO Fine-Tuning and Inference-Time Filtering}}
...
The experiment module implements the judge \changed{function of
Section~\ref{sec:dpo-judge}} and preference-data construction. It
...
plots used in \changed{Section~\ref{sec:dpo-judge} and in} this chapter.
...
\changed{MarioDPO is evaluated in two complementary ways. First, a January 2026
experiment snapshot, taken shortly after MarioDPO entered the arena, provides
a first evaluation against the established baselines. Second, the May 2026
PCG Arena export analysed in Chapter~\ref{ch:eda} provides the definitive
evaluation under the same blind pairwise-preference protocol as all other
generators.}

\subsection*{\changed{Initial evaluation (January 2026 snapshot)}}

Table~\ref{tab:dpo-controlled-results} summarises the \changed{January} snapshot. The
...
\changed{Because MarioDPO did not exist on the platform before deployment, all
of its evaluation battles necessarily post-date its training data; its
reported win rate and preference score are computed on votes disjoint from its
training pairs. Ratings of the baseline generators, by contrast, aggregate
votes from both periods; we return to this in the limitations
(Section~\ref{sec:conc-limitations}).}

\begin{table}[H]
\centering
\caption{\changed{Initial MarioDPO evaluation snapshot.} Win rate is computed from
...
\subsection*{\changed{Full arena evaluation (May 2026 export)}}
...
(0.643). \changed{MarioDPO thus approaches the human baseline and clearly
outperforms the other procedural generators in this export, suggesting that
preference-aligned generation is a promising direction for the platform.} The pairwise matrix in Chapter~\ref{ch:eda} further shows that MarioDPO
...
The change from an 83.3\% win rate in the \changed{January} snapshot to a 0.832
...
Second, the judge function \changed{(Section~\ref{sec:dpo-judge})} captures multiple dimensions of preference rather
...
\changed{This thesis addressed the central evaluation gap in procedural
content generation---the mismatch between scalable-but-inaccurate automated
metrics and valid-but-expensive human studies---through the design,
deployment, and exploitation of PCG Arena, a persistent web platform for blind
A/B testing of Super Mario Bros level generators.
\TODO{author: review and expand opening recap}}

\changed{\textbf{RQ1 (Measurement).} A persistent, blind, web-based arena can
...
(preference score 0.833) and pattern-based generators trail. Crucially,
anonymous players largely agree on which generators are good
(Section~\ref{sec:eda-user-heterogeneity}), so the ranking reflects shared
preference rather than individual taste. \TODO{author: review}}

\changed{\textbf{RQ2 (Explanation).} Player preference is explainable from
level structure and behaviour: reward density, unique-column ratio, and
proximity to the original corpus associate positively with preference, while
gap-heavy structure, concentrated death locations, and
\texttt{impossible}/\texttt{too\_hard} tags mark the dominant failure modes.
These correlates combine into the judge function $J_{final}$, whose
generator-level scores correlate with human win rates at $r_s = 0.736$
(Section~\ref{sec:dpo-judge}). \TODO{author: review}}

\changed{\textbf{RQ3 (Generation).} The collected preferences suffice to train
a better generator: MarioDPO, fine-tuned with DPO on 473 human pairs and 2,606
judge-labelled synthetic pairs, achieves a preference score of 0.832,
essentially tied with the human-authored baseline (0.833) and clearly ahead of
the next-best procedural generators, ORE (0.655) and MarioGPT (0.643).
\TODO{author: review}}

\changed{In sum, the thesis contributes: (1)~PCG Arena, a persistent
...
analysis pipeline linking votes to structure, behaviour, and tags; (4)~a
validated computational preference judge; and (5)~MarioDPO, a
preference-aligned generator that approaches human-authored quality.
\TODO{author: review}}
...
\changed{\begin{itemize}
    ...
    \item \changed{\textbf{Overlapping vote sets for baseline generators.} The
          May ratings of the baseline generators aggregate votes collected
          both before and after MarioDPO's deployment, so their training-time
          and evaluation-time vote sets partially overlap. MarioDPO itself is
          immune to this concern: its evaluation votes necessarily post-date
          its January training snapshot (Section~\ref{sec:dpo-results}).}
    \item \changed{\textbf{Judge bias in synthetic pairs.} The 2,606
          judge-labelled training pairs inherit any bias of $J_{final}$; the
          $10\times$ upweighting of human votes mitigates but does not
          eliminate this (Section~\ref{sec:dpo-discussion}).}
    \item \changed{\textbf{Engagement concentration.} The top 5 anonymous
          player IDs account for 54\% of the analysed votes, so aggregate
          scores partly reflect the preferences of a few highly active players
          (Section~\ref{sec:eda-user-heterogeneity}).}
\end{itemize}}
...
\changed{\begin{itemize}
    \item \textbf{Iterative retraining.} Train a MarioDPO-v2 on the May 2026
          ...
          continue to improve preference scores. \TODO{author: keep/expand}
    \item \textbf{Learned neural judge.} Replace the hand-weighted
          $J_{final}$ with a neural preference model trained end-to-end on the
          growing vote corpus. \TODO{author: keep/expand}
    \item \textbf{Beyond Mario.} Extend the arena protocol to other content
          ...
          procedural generators. \TODO{author: keep/expand}
    \item \textbf{Training-aware matchmaking.} Extend AGIS to actively select
          battles that maximise the value of votes as DPO training pairs, not
          only their ranking information. \TODO{author: keep/expand}
\end{itemize}}
