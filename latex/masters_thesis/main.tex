...
This chapter presents the PCG Arena data as a user study of player
...
The May 2026 export analysed in this chapter is the endpoint of a
...
Two clarifications are important for interpreting the analyses that
...
This agreement is what justifies the two uses the rest of the thesis
...
% ===================================================================
\section[Static Structure: A Coarse Filter, Not a Ranking]{Static Structure: A Coarse Filter, Not a Ranking}
\label{sec:eda-static-expressive-range}

This section asks what we can learn about a generator by
...
The table supports one clear reading: static structure acts as a
...
This answers RQ2 in the negative for ranking purposes: automated
...
% ===================================================================
\section[Diagnosing Failure: Trajectories and Tags]{Diagnosing Failure: Trajectories and Tags}
\label{sec:eda-trajectory-fingerprints}
\label{sec:eda-tag-semantics}

Static structure cannot explain the ranking, but the votes come with
...
The stacks read as a failure map. For the top generators the traces
...
% ===================================================================
\section[A Pragmatic Judge for Sparse Votes]{A Pragmatic Judge for Sparse Votes}
\label{sec:dpo-judge}
...
The last three quantities describe how a level is played, not how it
...
The weights (Table~\ref{tab:dpo-judge-weights}) were obtained by a
...
That, however, is exactly the granularity Chapter~\ref{ch:mariodpo}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Players agree}: the same generators score high or low across the anonymous population, so the aggregate ranking reflects shared preference rather than individual taste.
    ...
    \item \textbf{A coarse judge suffices for labelling}: combining the failure signals yields $J_{final}$, which reproduces the quality tiering in-sample ($r_s = 0.736$) by catching easy-to-spot failures, not by measuring enjoyment.
\end{enumerate}
...
The previous chapter delivered three results. First, blind pairwise
...
MarioDPO therefore combines three ingredients. A strong supervised
prior trained on the original Nintendo levels ensures the model already speaks
MarioDPO therefore combines three ingredients. Reusing the published
...
The MarioGPT checkpoint provides the grammar prior, PCG Arena provides sparse but
...
Rather than training a level generator from scratch, MarioDPO reuses
...
In the May 2026 PCG Arena export, MarioDPO is the strongest
...
(0.643). MarioDPO thus approaches the human baseline and clearly
...
Notably, MarioDPO also overtakes MarioGPT, the checkpoint it was
...
This thesis addressed the central evaluation gap in procedural
...
preference rather than individual taste. 
...
\begin{itemize}
    ...
    \item \textbf{Judge bias in synthetic pairs.} The 2,606
...
\begin{itemize}
    ...
    \item \textbf{Training-aware matchmaking.} Extend AGIS to actively select
          ...
          only their ranking information.
\end{itemize}
