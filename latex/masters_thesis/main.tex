...
\changed{The PCG Arena platform hosts \TODO{confirm: 14 or 15} generators
spanning all major paradigm families, 14 of which appear in the analyses of
this thesis, selected to benchmark a broad cross-section of established PCG
...
% AC(agent): deleted in M6 — the ERA-grid results figure and its intro
% paragraph duplicated Chapter 4 content; the results-bearing ERA figure
% lives only in Ch.4 (fig:eda-expressive-range).
...
% AC(agent): addressed in M6 — four-item enumerate condensed into one paragraph
% per restructure brief; all four citations, the KL-penalty idea, and the
% ROUGE/metric-mismatch point retained. The three \KOT comments below refer to
% the pre-condensation text and are kept for the reviewer.
...
capture summary quality, automated level metrics fail to capture fun,
...
\changed{This consistency licenses the two uses made of the vote data in the
remainder of the thesis: aggregating votes into a single preference ranking
...
The judge function is \changed{\emph{neuro-symbolic} in the sense that} it combines
...
Arena data. \changed{Three findings from the preceding sections
(Sections~\ref{sec:eda-static-expressive-range}--\ref{sec:eda-tag-semantics})
guide its design:}
...
\changed{MarioDPO is evaluated in two complementary ways
(Table~\ref{tab:eda-timeline}). First, a post-deployment snapshot taken in
January 2026, immediately after MarioDPO entered the arena, provides a
formative evaluation against the established baselines. Second, the May 2026
PCG Arena export analysed in Chapter~\ref{ch:eda} provides the summative
evaluation under the same blind pairwise-preference protocol as all other
generators.}

\subsection*{\changed{Post-deployment evaluation (January 2026 snapshot)}}

Table~\ref{tab:dpo-controlled-results} summarises the \changed{post-deployment} snapshot. The
...
\begin{table}[H]
\centering
\caption{\changed{Post-deployment evaluation snapshot.} Win rate is computed from
...
The change from an 83.3\% win rate in the \changed{post-deployment} snapshot to a 0.832
