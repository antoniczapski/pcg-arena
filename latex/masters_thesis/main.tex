...
disagreement on the mid-table generators. That agreement matters for the rest
of the thesis. It means the votes measure something shared rather than
individual taste, which is what makes it sound to combine them into a single
ranking (Section~\ref{sec:eda-rankings}) and to use them as a training signal
...
No column separates strong from weak generators either: the highest
...
replace human votes when the goal is to rank generators. This matches the gap
between metrics and player experience reported by
...
This limited ability is enough for the way Chapter~\ref{ch:mariodpo}
uses the judge. It is asked only one easy question at a time---which of two
clearly unequal levels is worse---so that training can include many examples
of what to avoid. The human votes, strongly upweighted, remain the only source
of information about what players actually enjoy. The judge's real test is
therefore downstream: whether the generator trained with its labels wins blind
...
structure before any preference optimisation begins. The 473 human
...
Three design choices explain the outcome.
...
Despite these limitations, MarioDPO closes the loop that the platform was
