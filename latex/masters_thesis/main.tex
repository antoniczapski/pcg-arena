...
Building on the collected data, we conduct an exploratory analysis that characterizes generator preference rankings, static expressive range, gameplay trajectory fingerprints, anonymous-user heterogeneity, and tag semantics. The analysis shows that PCG Arena functions not only as a leaderboard, but also as a diagnostic instrument for understanding why generators win or fail. \changed{It further shows that automated structural metrics cannot substitute for the human ranking, motivating training directly on player preferences.} Finally, we present MarioDPO, a level generator fine-tuned with Direct Preference Optimization that achieves a preference score of 0.832, approaching the hand-crafted original Nintendo baseline.
}
...
\begin{description}[style=nextline]
  ...
  \item[RQ2 (Metrics)] Can automated metrics substitute for the human ranking?
...
\noindent The thesis makes \changed{four} contributions:
...
preferences. PCG Arena's \emph{Judge Function} plays an analogous role \changed{on a
much smaller scale: a heuristic filter used to label additional level pairs
...
\changed{For researchers, the platform additionally exposes a per-level
analytics view (Figure~\ref{fig:eda-detailed-level-stats}) that combines the
level map with recorded player traces, death locations, and tag counts,
supporting qualitative inspection of individual levels.}

% MOVED(agent): figure relocated from old §4.5 (review pass), verbatim
\begin{figure}[H]
\centering
...
remaining sections address the metrics question (RQ2):
Sections~\ref{sec:eda-static-expressive-range}--\ref{sec:eda-tag-semantics}
test whether static level structure, gameplay trajectories, and subjective
tags can explain or replace the ranking, and Section~\ref{sec:dpo-judge}
builds a pragmatic judge that Chapter~\ref{ch:mariodpo} uses to label
...
% ===================================================================
\section[Static Structure: A Coarse Filter, Not a Ranking]{\changed{Static Structure: A Coarse Filter, Not a Ranking}}
...
\changed{Table~\ref{tab:eda-static-metrics} reports these metrics per
...
\bottomrule
\end{tabular}
\caption{\changed{Static expressive metrics by generator (May 2026 export),
...
\changed{The table supports one clear reading: static structure acts as a
coarse filter, not a ranking. The four highest-scoring generators have similar
profiles---linearity between 0.02 and 0.11, leniency between 0.55 and 0.67,
gap density between 0.14 and 0.22, enemy density between 0.06 and
0.09---yet their preference scores span 0.64 to 0.83; nothing in the static
columns anticipates that players prefer \texttt{original} and
\texttt{mariodpo} over \texttt{ore} and \texttt{mariogpt} by almost 0.2.
Conversely, no column separates strong from weak generators: the highest
...
(0.44) is the lowest in the corpus---but such red flags identify obvious
failures rather than order the plausible generators.}

\changed{This answers RQ2 in the negative for ranking purposes: automated
structural metrics cannot substitute for human votes, consistent with the
metric--experience misalignment reported by
\citet{marino2015empirical}. Figure~\ref{fig:eda-expressive-range} makes the
same point visually: each generator occupies a recognisable region of the
linearity--leniency plane, so the metrics do \emph{characterise}
generators---but neither the location nor the spread of these footprints
follows the preference ranking.}
...
% ===================================================================
% AC(agent): review pass — old §4.5 (trajectories) and §4.6 (tags) merged;
% trajectory-metrics figure dropped; analytics-UI figure moved to Ch. 3.
\section[Diagnosing Failure: Trajectories and Tags]{\changed{Diagnosing Failure: Trajectories and Tags}}
\label{sec:eda-trajectory-fingerprints}
\label{sec:eda-tag-semantics}

\changed{Static structure cannot explain the ranking, but the votes come with
two further data layers that can explain \emph{losses}: where players fail
(gameplay trajectories) and what they report (tags).}
...
\changed{The stacks read as a failure map. For the top generators the traces
extend far into the level and cover a broad vertical band: the median maximum
progress is 1126.5 pixels for \texttt{original} and 988.0 pixels for
\texttt{mariodpo}. For the weakest generators the traces pile up and terminate
...
bin. Players facing these levels do not gradually lose interest---they hit a
wall within the first screens.}
...
\changed{Trajectories and tags converge on the same conclusion. The dominant,
diagnosable driver of lost battles is \emph{frustration}: levels that kill the
player early and repeatedly in the same places are the levels tagged
\texttt{impossible} and \texttt{too\_hard}, and they lose. Positive quality
behaves differently: \texttt{fun} and \texttt{creative} align with winning,
but Section~\ref{sec:eda-static-expressive-range} found no structural recipe
for them. This asymmetry---failure is easy to detect, enjoyment is
not---shapes the judge function of the next section.}

% ===================================================================
% AC(agent): review pass — Experiments A–D and their figures removed; judge
% presented as a pragmatic failure filter validated in-sample only.
\section[A Pragmatic Judge for Sparse Votes]{\changed{A Pragmatic Judge for Sparse Votes}}
\label{sec:dpo-judge}

\changed{Chapter~\ref{ch:mariodpo} trains a generator on preference pairs, and
the 473 usable human votes of the January training snapshot
(Table~\ref{tab:eda-timeline}) are far too few for that purpose on their own.
We therefore need an automatic way to label additional level pairs. The
preceding sections dictate what such a labeller can and cannot be: no
combination of static metrics reproduces the human ranking
(Section~\ref{sec:eda-static-expressive-range}), so we do not attempt to model
enjoyment. What \emph{is} reliably measurable is failure
(Section~\ref{sec:eda-tag-semantics}). The judge function $J_{final}$ is
therefore a deliberately coarse filter: it separates obviously flawed levels
from plausible ones rather than scoring how much fun a level is.}

\changed{The judge is a hand-weighted sum of five interpretable terms:}
%
\begin{equation}
...
\end{equation}
%
\changed{where $D_M$ is the Mahalanobis distance from a level's feature vector
...
fragmented, pit-heavy ground; $\mathit{Completion}$ is the observed completion
rate; $\sigma_y$ is the standard deviation of Mario's vertical position in an
A* or player trajectory; and $\mathit{Hesitation}$ is the fraction of
trajectory time spent at near-zero horizontal velocity. For freshly generated
levels with no play data, the trajectory quantities are replaced by static
proxies computed from the tile grid (surface roughness for $\sigma_y$, the
structural gap fraction for the penalty). The weights
(Table~\ref{tab:dpo-judge-weights}) were set by hand. The gap penalty
dominates because frustration is the one failure mode the data identifies
unambiguously (Section~\ref{sec:eda-tag-semantics}); the remaining terms
encode the softer observations of
...
$w_{comp}$  & 0.20 & reward observed completion \\
$w_{flow}$  & 0.10 & penalise hesitation and poor flow \\
\bottomrule
\end{tabular}
\caption{\changed{Judge-function weights (hand-set).}}
\label{tab:dpo-judge-weights}
\end{table}

\changed{Aggregated by generator on the January snapshot, $J_{final}$
correlates with human win rate at $r_s = 0.736$ ($p = 0.0027$). This number
must be read carefully: it is an in-sample consistency check over 14
generators, not out-of-sample predictive power. The weights were chosen on the
same data, the completion term is derived from observed play, and in this run
the gap penalty fell back to a pattern-generator indicator because reliable
per-level gap features were unavailable. What the correlation shows is that
the judge reproduces the coarse quality tiering---it reliably places the
degenerate, frustrating generators below the plausible ones. It catches
easy-to-spot failures; it does not measure the fun of playing.}

\changed{That, however, is exactly the granularity Chapter~\ref{ch:mariodpo}
needs: the judge is used only to label clearly-unequal level pairs, supplying
broad negative coverage against known failure modes, while the sparse human
votes---strongly upweighted---remain the sole carrier of the preference
signal. The judge's real test is therefore downstream: whether the generator
trained with its labels wins blind human votes it has never seen.}

% ===================================================================
\section[Summary: A Reliable but Sparse Signal]{\changed{Summary: A Reliable but Sparse Signal}}
\label{sec:eda-summary}

\changed{The chapter's findings form a single argument:}
\begin{enumerate}[nosep]
    \item \changed{\textbf{Measurement works}: blind pairwise votes produce a stable ranking, with \texttt{original} and \texttt{mariodpo} nearly tied at the top ($\approx$0.83) and pattern-based generators at the bottom.}
    \item \changed{\textbf{Players agree}: the same generators score high or low across the anonymous population, so the aggregate ranking reflects shared preference rather than individual taste.}
    \item \changed{\textbf{Static metrics cannot rank}: the top generators have near-identical static profiles and no metric column separates strong from weak; structure acts only as a coarse filter for degenerate designs.}
    \item \changed{\textbf{Failure is diagnosable}: early, concentrated deaths and \texttt{impossible}/\texttt{too\_hard} tags mark losing levels, while \texttt{fun} and \texttt{creative} align with wins but show no structural recipe.}
    \item \changed{\textbf{A coarse judge suffices for labelling}: combining the failure signals yields $J_{final}$, which reproduces the quality tiering in-sample ($r_s = 0.736$) by catching easy-to-spot failures, not by measuring enjoyment.}
\end{enumerate}

\changed{Two properties of the vote signal matter for what follows. It is
...
artefact of individual taste. It is also \emph{sparse}: the
...
\changed{The previous chapter delivered three results. First, blind pairwise
...
no automated metric substitutes for that signal---player enjoyment has no
simple structural proxy
(Section~\ref{sec:eda-static-expressive-range})---which is precisely why
MarioDPO is trained on the preferences themselves rather than on a
hand-crafted fitness function. Third, although enjoyment resists measurement,
failure does not: frustrating, uncompletable structure is reliably
identifiable, and the judge function of Section~\ref{sec:dpo-judge} labels it
cheaply and at scale. What the arena does \emph{not} deliver is volume: the
January 2026 training snapshot contains only 473 usable non-tie
votes---orders of magnitude below the preference datasets used to align
language models (e.g., the 64K comparisons of \citet{stiennon2020learning})
and far too few to fine-tune a level generator on preference alone.}
...
pairs, upweighted $10\times$, anchor training to genuine human
judgement---they are the only carrier of the ``fun'' signal. And the judge
function labels 2,606 additional synthetic pairs, providing the broad negative
coverage needed to steer the model away from known failure modes without
...
is lower. \changed{This is not a contradiction: the judge is a coarse failure
filter, not a measure of enjoyment (Section~\ref{sec:dpo-judge}); human votes
remain the final criterion.} The discrepancy suggests
...
Second, the judge function \changed{(Section~\ref{sec:dpo-judge}) supplies
broad negative coverage rather than a model of enjoyment. It reliably flags
the failure modes identified in Chapter~\ref{ch:eda}---gap-fragmented,
uncompletable, structurally degenerate levels---so DPO sees many unambiguous
...
\changed{reproduces the coarse quality tiering}, but it cannot capture nostalgia,
...
\changed{\textbf{RQ2 (Metrics).} No. Automated structural metrics act as a
...
no metric column separates strong from weak generators
(Section~\ref{sec:eda-static-expressive-range}). What is measurable is
failure: frustrating, uncompletable structure shows up consistently in
trajectories and tags (Section~\ref{sec:eda-tag-semantics}). A pragmatic judge
built on these failure signals reproduces the coarse quality tiering in-sample
($r_s = 0.736$) and suffices to synthesise training labels, but it detects
easy-to-spot failures rather than measuring enjoyment.
\TODO{author: review}}
