...
\changed{The judge is a weighted sum of five interpretable terms:}
%
\begin{equation}
\label{eq:dpo-j-final}
\changed{\begin{aligned}
...
\changed{The last three quantities describe how a level is played, not how it
looks, so they cannot be read off the tile grid. Because the judge must score
freshly generated levels that no human has played, they are obtained by
simulation: an A* agent is run through each candidate level, and its trace
supplies the completion, verticality, and hesitation values. Using an agent as
a stand-in for a player is a well-established practice in PCG evaluation
(Section~\ref{subsec:bg-eval-agents}), and it inherits the bias discussed
there---the agent is not a human---which is a further reason to treat the
judge as a coarse filter rather than a model of enjoyment.}

\changed{The weights (Table~\ref{tab:dpo-judge-weights}) were obtained by a
grid search that maximises pairwise agreement with the human votes, subject to
fixed signs for each term so that the resulting score stays interpretable. The
gap penalty ends up dominating, consistent with frustration being the one
failure mode the data identifies unambiguously
(Section~\ref{sec:eda-tag-semantics}); the remaining terms
...
\bottomrule
\end{tabular}
\caption{\changed{Judge-function weights, obtained by grid search over
...
generators, not out-of-sample predictive power---the weights were selected on
the same votes it is evaluated against. What the correlation shows is that
...
\changed{MarioDPO therefore combines three ingredients. Reusing the published
MarioGPT checkpoint ensures the model already speaks
...
\begin{enumerate}[nosep]
  \item a column-major level representation that preserves the vertical structure
        ...
        sequence modelling;
  \item \changed{a base generator reusing the MarioGPT checkpoint};
...
\changed{deterministic post-processing
(Section~\ref{subsec:dpo-training-details})}. Preference
...
\changed{The MarioGPT checkpoint provides the grammar prior}, PCG Arena provides sparse but
...
to 193.8 tiles. \changed{The vocabulary contains 37 distinct tile characters,}
...
\changed{Post-processing acts on the same column units when enforcing ground
...
\changed{Training examples are obtained by sliding a window over the
...
\subsection[Base Model]{\changed{Base Model}}
\label{subsec:dpo-sft}

\changed{Rather than training a level generator from scratch, MarioDPO reuses
...
reachable space, enemies on supporting tiles---before any preference
optimisation begins. Starting from this checkpoint means the DPO stage only has
to move the policy towards preferred levels rather than rediscover basic Mario
syntax.}
...
setting, \changed{the prompt $x$ is a level prefix}, while
$y_w$ and $y_l$ are the preferred and rejected level continuations. \changed{The
...
\textbf{Parameter} & \textbf{Value} \\
\midrule
Base model & \changed{MarioGPT checkpoint (DistilGPT-2)} \\
Vocabulary & 37 ASCII tile characters \\
Context length & \changed{1024 tokens} \\
Batch size & 32 \\
DPO $\beta$ & 0.1 \\
Learning rate & $10^{-5}$ \\
Epochs & 3 \\
...
last columns, and that both regions contain solid ground. \changed{The candidates
are then scored with} $J_{final}$, and the highest-scoring candidate is selected. This ``safety net''
...
\changed{MarioDPO is evaluated in the May 2026 PCG Arena export analysed in
Chapter~\ref{ch:eda}, under the same blind pairwise-preference protocol as all
other generators.}

In the \changed{May 2026} PCG Arena export, MarioDPO \changed{is} the strongest
...
\caption{MarioDPO compared with other procedural generators\changed{: it is the
...
\changed{Notably, MarioDPO also overtakes MarioGPT, the checkpoint it was
initialised from: preference alignment on arena votes, rather than a change of
architecture or training corpus, accounts for the difference. This places
MarioDPO in the same top tier as the original SMB1 levels and above all other
algorithmic generators evaluated in PCG Arena.}
...
First, the base model starts from a strong structural prior. \changed{Reusing the
...
\changed{\begin{itemize}
    ...
    \item \textbf{Learned neural judge.} Replace the \changed{grid-searched}
...
\section[Closing Remarks]{\changed{Closing Remarks}}
\label{sec:conc-closing}

\changed{The lesson of this thesis is that human taste does not
compress into a simple mathematical expression. Chapter~\ref{ch:eda} shows
this directly: the generators players like best are structurally almost
indistinguishable from ones they like considerably less, and no metric column,
expressive-range footprint, or hand-built score separates them. Even the judge
function of Section~\ref{sec:dpo-judge}, built specifically for this corpus,
succeeds only at recognising obvious failures---it never becomes a measure of
enjoyment.}

\changed{This offers an explanation for a long-standing pattern in the field.
...
optimised was a guess about human preference rather than a measurement of it.}

\changed{MarioDPO takes the opposite route. It never represents ``fun''
...
\changed{Nothing in this pipeline is specific to Super Mario Bros. It requires
