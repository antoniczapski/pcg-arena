...
\begin{description}[style=nextline, leftmargin=1.5em]
    ...
    Decision Process; train an agent to construct content. Requires no training
    corpus but depends on reward design.
\end{description}
...
subjects to play \emph{Infinite Mario Bros} variants and report pairwise
affective preferences; a follow-up by \citet{shaker2011features} collected 600
game-pair comparisons via an uncontrolled web applet, but the number of unique
participants is not reported and the study predates the recent wave of
ML-based generators (GANs, LSTMs, diffusion models) that have since reshaped
the field.
...
\end{align}
%
where $g(\phi) = 1/\sqrt{1 + 3\phi^2/\pi^2}$ attenuates predictions for
uncertain opponents, $s_j$ is the observed outcome, and $E_j$ the expected
outcome.
...
\emph{Paradox}, treating both players and puzzle-tasks as rated entities. This is
the most direct precedent for PCG Arena's design, demonstrating that
player-rating systems can simultaneously estimate human skill and task
difficulty.
