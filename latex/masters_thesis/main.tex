...
The game loop runs at 30~ticks per second (matching the original framework), with
rendering at 60~FPS via \texttt{requestAnimationFrame}. Physics updates are fully
...
World~1-1 is encoded as a $202 \times 16$ character grid.
...
To address these shortcomings, we developed \textbf{AGIS (Adaptive Glicko-Informed
Selection)}, a two-stage matchmaking algorithm that incorporates rating uncertainty,
skill similarity, and pairwise coverage into generator selection.
...
where $q_{\text{bias}} = 0.2$ is the configurable quality bias strength. The first
generator is then sampled from the resulting categorical distribution.
...
Each user may own at most three generators. Generators that have participated in battles
are soft-deleted (marked inactive and excluded from matchmaking) to preserve historical
data integrity; generators without any battles can be hard-deleted.
