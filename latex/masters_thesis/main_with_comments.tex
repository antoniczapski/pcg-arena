...
\polishtitle    {PCG Arena: Platforma do zaślepionego Testowania A/B\fmlinebreak Generatorów Treści Proceduralnych\fmlinebreak z Optymalizacją Preferencji}
...
algorithm produced a level, expectations contaminate their judgments. \KOT{I suppose how the original Mario PCG competition worked? exactly as your study - play two levels choose best?}
...
Mario level generators. We \KOT{primarely} investigate three research questions:
...
content generation \KOT{(in the context of Maio level generation?)}, evaluation methodology, rating systems, and preference
...
\KOT{trche nie wiem jak z tych obu papierów można powiedzxieć że jest takich 5 paradygmatów generacji... przejrzałem je teraz tylko żeby sobie przypomniec ale mi sie to toche nie spina z tekstami źródłowymi}
...
(ORE)~\citep{mawhorter2010occupancy} \KOT{tu i w kolejnych akapitach brakuje mi 'that' do spoójności przedstawiania co robią generatory względem wcześniejszego tekstu} builds levels by iteratively attaching
...
summerville2017understanding} use $R^2$ \KOT{co to za wzorek, po co te literki i nazwy, to nic nie mowi, nic nie tłumaczy} goodness-of-fit instead, inverting the
...
scales for enjoyment, aesthetics, and difficulty ($n = 37$)\KOT{wtf is n?}.
...
These studies collectively demonstrate that while human empirical evaluation provides the
...
\KOT{więcej tego jeszcze było z human studies: https://ojs.aaai.org/index.php/AIIDE/article/view/12399/12258 oraz http://julian.togelius.com/Dahlskog2013Patterns.pdf  ze slajdów złapałem co mi sie przypomniało}
...
and then updates RD and rating: \KOT{chyba coś nie jest wprowadzone - czym jest phi?}\KOT{ok, zobaczyłem potem ranking deviation wprowadzając gaussiana dałeś jako sigmę square a teraz używasz phi...}
%
\begin{align}
...
\begin{enumerate}[nosep]
    ...
    better than ROUGE.\KOT{co, kto, z czym w ogóle o co chodzi>}
...
\texttt{ratings}             & Current Glicko-2 state ($\mu$, RD, $\sigma$\KOT{sigma nie powinna mieć literki?}) per generator \\
...
The porting process involved four main translation steps: \KOT{jakaś notka o porvcie? na ile ręcznie na ile automatem jakimś, jak tak to jakim i czy były potrzebne jakieś fixy? Informacje o tym powinny się znaleźć w pracy}
...
\KOT{tego i w ogóle opisu samej gry, poweróów też? etc brakuje wcześniejw  backgroundzie, tak naprawdę nawet przed related Work, takie jest przynajmniej moje zdanie, nie wiem jak Sie Kindze czytało}
...
shows the segment of this level:\KOT{Jak dajesz ef do figure to już bez dwukropka. I BTW fajnie jakby obok była postawiona kolorowa wersja ;]}
...
\begin{enumerate}
    ...
    \item \textbf{Width bounds:} the number of columns must satisfy
          ...
          with widths between 150 and 208 columns; the lower bound of 1\KOT{??} is deliberately
...
\begin{figure}[H]
    ...
    \caption{Voting panel shown after both levels are played, with preference buttons
             and optional tag selection.\KOT{tag selection part is not visible onthis screen actually}}
    \label{fig:voting}
\end{figure}
...
\text{RD}_{\min})$ is the normalised rating deviation. The boost\KOT{boost też zmienionym fontem} function provides a $4\times$
...
\KOT{chyba tym wszystkim screenom by się przysłuzył clip do środkowej meaningful części. Tzn ja lubię zielony, połacie zielonego po bokach mi nie przeszkadzają, ale jednak readability się pogarsza o nie pozwala trochę powiększyć obrazka żeby coś było widać na powiększeniu mniejszym niż 180}
