...
2 & \textbf{mariodpo} & \textbf{83.3\%} & \textbf{0.55} & \textbf{40.0\%} \\
...
The hand-crafted ``original'' levels dominate with a 89.6\% win rate, serving as the benchmark. Remarkably, \textbf{MarioDPO achieves 83.3\% win rate}, ranking second overall and first among PCG generators---surpassing all other neural/ML methods. Among other procedural generators, ``ore'' and ``notch'' perform best (67-69\%), while pattern-based generators consistently rank at the bottom ($<$27\%).
...
\textbf{Results}: Spearman correlation $r = -0.227$, $p = 0.046$. No inverted-U pattern was detected; instead, a significant linear relationship confirms that easier levels are simply preferred.
...
1 ``Mainstream'' & 7 & 441 & 0.82 & 14.7s & 16.4\% & 10.8 \\
...
\textbf{Metric} & \textbf{Value} \\
\midrule
Mahalanobis Distance vs Win Rate ($r$) & -0.279 \\
p-value & \textbf{0.011} \\
Style Reward $\frac{1}{1+D_M}$ vs Win Rate ($r$) & +0.279 \\
Original mean $D_M$ & 1.80 \\
Pattern generators mean $D_M$ & $\approx$15.3 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Conclusion}: \textbf{Strongly Supported}. Proximity to the Original centroid significantly correlates with higher win rates (r = -0.28, p = 0.011). Pattern generators are furthest from the Original style ($D_M \approx 14.5$), explaining their poor performance. The Judge should include a \textbf{style reward} proportional to $\frac{1}{1+D_M}$.
...
\begin{enumerate}
    \item \textbf{MarioDPO Achieves SOTA}: Our DPO-aligned generator achieves 83.3\% win rate, ranking second only to hand-crafted Original levels and surpassing all other neural/ML methods.
    ...
    \item \textbf{Exploration Matters}: Levels enabling spatial exploration are preferred. Good PCG should create non-linear, explorable spaces.
    
    \item \textbf{Hand-Crafted Benchmark}: ``Original'' levels win 89.6\% of comparisons, setting a high bar for procedural generators to match.
    
    \item \textbf{Tag Validity}: Player tags correspond to measurable differences---``fun'' correlates with completion, ``creative'' with higher win rates.
    
    \item \textbf{Pattern Methods Fail}: All ``pattern*'' generators rank lowest (20-26\%), suggesting pattern-based approaches produce technically valid but aesthetically poor levels.
