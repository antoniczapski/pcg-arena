...
\label{subsec:platform-stats}

As a newly deployed platform, PCG Arena is in its initial data collection phase. The platform infrastructure has been fully validated through:

\begin{itemize}
    \item End-to-end testing of battle flow (request $\rightarrow$ play $\rightarrow$ vote $\rightarrow$ rating update)
    \item Automated demo scripts running 10+ battles with random votes
    \item Manual testing across multiple browsers (Chrome, Firefox, Safari, Edge)
    \item Load testing confirming responsiveness under concurrent users
\end{itemize}

Current seeded generators include:
\begin{itemize}
    \item \textbf{Notch}: Original Super Mario Bros. level design style
    \item \textbf{Hopper}: Pattern-based generation
    \item \textbf{Genetic}: Evolutionary algorithm-based generation
\end{itemize}

Each generator is seeded with 10+ levels for initial testing, with access to larger level sets (1000+ levels per generator) from the Mario AI Framework available for future expansion.

\subsection{Preliminary Generator Rankings}
\label{subsec:rankings}

Initial rankings reflect the starting state (all generators at 1000 $\pm$ 350) plus any early votes. As more battles are completed, we expect to observe:

\begin{enumerate}
    \item \textbf{RD Convergence}: Rating deviations should decrease from 350 toward 30--100 as confidence increases.
    \item \textbf{Rating Separation}: Distinct quality differences between generators should emerge.
    \item \textbf{Stable Rankings}: After sufficient battles, rank order should stabilize.
\end{enumerate}

The Glicko-2 system's uncertainty quantification enables statistical tests for whether observed differences are significant.

\subsection{User Experience Insights}
\label{subsec:ux-insights}

Based on initial testing and informal feedback:

\textbf{Strengths:}
\begin{itemize}
    \item Browser-based gameplay removes download barrier---users can participate immediately
    \item Side-by-side comparison format is intuitive
    \item Tagging system captures nuanced feedback beyond simple preferences
    \item Real-time leaderboard updates provide immediate gratification
\end{itemize}

\textbf{Areas for Improvement:}
\begin{itemize}
    \item Mobile/touch support not yet implemented (desktop-only)
    \item Level difficulty can vary significantly, affecting fair comparison
    \item Some users find 30-second levels too short for assessment
    \item Tutorial for first-time users would improve onboarding
\end{itemize}

These insights inform future development priorities.
