...
**Citation Key:** `yannakakis2011experience`
...
their demographic diversiﬁcation [1]. Twenty years ago, game
...
becomes increasingly difﬁcult. Game engines that are able to
...
section we deﬁne the fundamental elements of the ExperienceDriven Procedural Content Generation (EDPCG) framework
...
manifestations. In our deﬁnition player experience is the
...
itself. This deﬁnition includes such aspects as maps, levels,
stories, quests, characters, rulesets, camera proﬁles, music,
sound effects, and weapons. Content in games can be seen
through the lens of ﬁve facets of human (or computational)
creativity: game design, level architecture, visuals, audio, and
narrative [5].
Procedural content generation (PCG) refers to the creation of game content — as deﬁned above — automatically
...
has bloomed only in the last ﬁve years. Beyond games, PCG
...
EDPCG framework [3] is deﬁned as a generic and effective
...
justiﬁability of affect-based, automated, game decisions —
are eliminated [4]. Games are designed to offer affective
experiences inﬂuenced by player feedback and many games let
...
alterations of the interaction, and this inﬂuences the game
...
one needs to fulﬁll a set of system requirements: the game
...
player input. The EDPCG framework satisﬁes these conditions
via the efﬁcient generation of game content which is driven
...
content to computational models of user affect and, ﬁnally, it
...
and game artiﬁcial intelligence. While EDPCG is a framework
...
in [3], it highlights its inﬂuence to variant research areas, and
...
EDPCG deﬁnes a novel approach to PCG coupling player
...
• Content representation: content is represented accordingly to maximize search efﬁcacy and robustness.
...
of data collected from players which in turn deﬁne different
...
obtained data. Subjective PEM considers only ﬁrst person
...
self-reports can be classiﬁed as rating, in which the players
...
of this approach. First, there are usually signiﬁcant subjectivity
...
changes in the player’s physiology, reﬂect on the player’s facial
...
in which bodily responses are mapped to speciﬁc emotional
...
head, ﬁngertips) and sensors, making physiological signals
...
focus. Cognitive processes may, in turn, inﬂuence emotions;
...
be classiﬁed as model-based, model-free or some hybrid between the two. Model-based approaches are typically inspired
...
are also theories about user affect that are speciﬁc to games
2http://emotiv.com
...
task have been used in the literature, but also game-speciﬁc
measures such as the weapons selected in a shooter game or
the times a player dies. Moreover, several different difﬁculty
and challenge measures have been proposed for different game
genres (see [28], [29] among many). Finally, a player proﬁle
or a player model [30] can be embedded in the process of
gameplay-based PEM.
Gameplay-based PEM is certainly the most computationally
efﬁcient and least intrusive PEM approach of all three but it
...
machine learning (regression and classiﬁcation) algorithms can
...
include decision trees, artiﬁcial neural networks (shallow and
deep architectures) and support vector machines [32].
B. Content Quality
In EDPCG, the main use of the acquired player models is to
judge the quality (usefulness, ﬁtness) of game content items.
...
scalar (or a vector of real numbers) that accurately reﬂects its
...
an ill-posed problem; the designer ﬁrst needs to decide what,
...
number of paths to the exit in a maze, the ﬁring rate of a
...
and is often speciﬁcally tailored to the particular game and
...
ﬁnally to evaluation functions [37], [38], [39].
...
in the game, which means that ﬁtness is evaluated during
...
physiological response, gaze ﬁxation, speech quality, facial
expressions and postures. Data is used to tailor the player
experience models to the speciﬁc player, which in turn affects
...
the function is classiﬁed as explicit; otherwise (if coupled with
objective or gameplay-based PEM) the function is classiﬁed as
...
artiﬁcial neural networks might allow for more effective search
...
speciﬁc part of the phenotype, and indirect encodings, wherein
...
ground, coin, wall, enemy, free space) is speciﬁed separately,
...
We take our example from Pedersen et al. [39], who modiﬁed
...
The ﬁrst step was to represent the levels in a format that
...
competition.
Fig. 3.
The EDPCG framework in detail. The gradient grayscale-colored
...
content generation and has also inﬂuenced the larger areas of
affective interaction and game artiﬁcial intelligence. The use
...
inﬁnite replayability value increases substantially. The automation of content creation, however, offers a unique opportunity
...
Even though EDPCG has inﬂuenced the above research
ﬁelds at large there are several interesting research questions
that require much more study. These include the appropriate
representation of game content and the design of relevant and
computationally efﬁcient evaluation functions based on reliable computational models of player experience. The potential
gains from providing good solutions to these challenges are
signiﬁcant: the invention of new game genres built on PCG,
...
Players.
MIT Press, 2009.
[2] T. Taylor, Play Between Worlds: Exploring Online Game Culture.
The
...
2015.
[8] R. W. Picard, Affective Computing.
Cambridge, MA: The MIT Press,
1997.
[9] P. Sundstr¨om, “Exploring the affective loop,” Stockholm University,
Tech. Rep., 2005.
[10] G. N. Yannakakis and J. Togelius, “A panorama of artiﬁcial and
...
Interaction.
Amsterdam, The Netherlands: IEEE, September 2009, pp.
126–131.
...
Handbook.
Springer-Verlag Berlin Heidelberg, 2011, ch. Image and
...
Research Workshop on Perception and Interactive Technologies.
Germany: Springer-Verlag Berlin Heidelberg, 2006, pp. 53–64.
...
instructional computer games,” in Proceedings of the 3rd ACM SIGSMALL symposium and the ﬁrst SIGPC symposium on Small systems,
1980, pp. 162–169.
[26] R. Koster, A theory of fun for game design.
Paraglyph press, 2005.
[27] A. Drachen, C. Thurau, J. Togelius, G. N. Yannakakis, and C. Bauckhage, “Game data mining,” in Game Analytics.
Springer, 2013, pp.
205–253.
[28] G. van Lankveld, P. Spronck, and M. Rauterberg, “Difﬁculty Scaling
through Incongruity,” in Proceedings of the 4th International Artiﬁcial
...
[30] G. N. Yannakakis, P. Spronck, D. Loiacono, and E. Andr´e, “Player
modeling,” Artiﬁcial and Computational Intelligence in Games, vol. 6,
pp. 45–59, 2013.
[31] J. F¨urnkranz and E. H¨ullermeier, “Preference learning,” K¨unstliche
Intelligenz, vol. 19, no. 1, pp. 60–61, 2005.
[32] V. E. Farrugia, H. P. Mart´ınez, and G. N. Yannakakis, “The preference
...
procedural map generation,” in Proceedings of the Workshop on Procedural Content Generation, Foundations of Digital Games.
Monterey,
...
Perth, Australia: IEEE, December 2008, pp.
252–259.
...
[38] O. Missura and T. G¨artner, “Player modeling for intelligent difﬁculty
...
of AIIDE.
Palo Alto, CA: AAAI Press, October 2010, pp. 63–68.
