...
[48]. However, current approaches in this field of Procedural Con-
...
arXiv:2302.05981v2  [cs.AI]  16 Jun 2023
...
ter generation, environment modeling, etc.). As reviewed in [39, 49],
...
ticularly for Super Mario [3, 11, 36–38, 48], also leveraged neural
network architectures to create levels. Beukman et al. [3] evolved
neural networks in order to generate levels, while [11, 15, 37, 48]
...
15, 37, 48] also explored the abilities to control characteristics in gen-
...
allowing for easily controllable level generation. In other words,
instead of searching for a level with specific characteristics, Mari-
oGPT allows us to just ask for it. Concurrently to our work, Todd
et al. [45] showed that LLMs can also be used to generate levels for
other games such as Sokoban, thus suggesting the applicability of
this approach to many different games in the future.
2.3
...
temporal interdependency of the operations. Transformers [47]
...
novelty-search evolutionary loop (Figure 3) in Section 3.4, allowing
...
Path
x
Mario levels are represented similarly to previous works [11, 15,
36–38, 48], using the levels provided in the Video Game Level Cor-
pus (VGLC) [43]. We utilize a relatively small set of path-annotated
...
els previously in use for procedural content generation [41, 48]),
...
[23]. In total, MarioGPT sees 200,000 training samples. Because the
model is relatively small, it can be trained using a single Nvidia
2080ti GPU.
...
illustrated in Figure 2. Prompts are encoded through BART [28],
a frozen pre-trained language model. Prompts are passed through
...
model.
Prompts are represented as combinations of specific features (e.g.
...
[11, 48]) and requires the use of an external agent for evaluation.
...
91%
YES
(1) Tile Prediction Accuracy: To measure how proficient Mari-
oGPT is in generating levels, we compare prediction accuracy com-
...
[22, 46] in 250 generated levels1. We find that 88.33% of all MarioGPT-
...
(d) "no pipes, no enemies, many blocks"
(e) Prompt that does not exist in the dataset: "many pipes, no enemies, many blocks"
...
(d) Playable level with interesting path trajectories
...
search (Section 3.4), we are able to continuously generate diverse
...
Figure 9: Comparison of most and least novel levels in the archive. The two least novel levels are very similar to each other, while the
most novel levels have more distinct path patterns.
Figure 10: tSNE of the Archive Elements. tSNE embeddings
are computed from the behavioral characteristic. Darker points
indicate more recently added elements. Although novelty search is
using the behavioral characteristics of the player paths, the levels
also demonstrate visual novelty.
...
learning from human feedback (RLHF) [50]. The ability to fine-tune
...
[45] Graham Todd, Sam Earle, Muhammad Umair Nasir, Michael Cerny Green, and
...
,
Sudhakaran et al.
[46] Julian Togelius, Sergey Karakovskiy, and Robin Baumgarten. 2010. The 2009
...
[48] Vanessa Volz, Jacob Schrum, Jialin Liu, Simon M. Lucas, Adam Smith, and Sebas-
...
[49] Georgios N Yannakakis and Julian Togelius. 2018. Artificial intelligence and games.
Vol. 2. Springer.
[50] Daniel M. Ziegler, Nisan Stiennon, Jeffrey Wu, Tom B. Brown, Alec Radford,
