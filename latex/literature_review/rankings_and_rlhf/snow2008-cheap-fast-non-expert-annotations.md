...
experts.
We propose a technique for bias
correction that signiﬁcantly improves annota-
...
annotations. We chose ﬁve natural language under-
standing tasks that we felt would be sufﬁciently nat-
...
learning classiﬁers trained on expert annotations vs.
non-expert annotations.
In the next sections of the paper we introduce
the ﬁve tasks and the evaluation metrics, and offer
...
going public discussion.
We encourage comments to be di-
rected here in addition to email when appropriate.
Dolores
...
sense word relations (Singh, 2002) sufﬁciently “easy
...
ity against experts, ﬁnding validation tests were im-
...
without external gold standard comparisons.
In
...
ilar to our ﬁndings; their summaries of worker be-
...
ing classiﬁers. Since we focus on empirically val-
...
set of qualiﬁcations, such as sufﬁcient accuracy on
...
a ﬁxed range. For every task we collect ten inde-
...
other ﬁve annotators. For each expert labeler, we
computed this ITA score of the expert against the
other ﬁve; we then average these ITA scores across
...
ing Pearson correlation across all sets of the ﬁve ex-
...
ﬁrst comparison showed that individual experts were
...
compute the ITA with respect to each subset of ﬁve
...
across all 7 tasks we ﬁnd that on average it re-
...
enough, 1965).
Speciﬁcally, we ask for numeric
...
4(Miller and Charles,
1991) and others originally used a
...
izontal line; we ﬁnd that at 10 annotators we achieve
...
inferred from the ﬁrst. For example, the hypothesis
...
implement temporal ordering as a simpliﬁed version
...
described by the ﬁrst verb occurs before or after the
second. For example, in a dialogue about a plane
explosion, we have the utterance: “It just blew up in
the air, and then we saw two ﬁreballs go down to the,
...
tation we highlight the speciﬁc verb pair of interest
(e.g., go/coming, or blew/saw) and ask which event
occurs ﬁrst (here, go and blew, respectively).
...
ITA numbers have been reported for this simpliﬁed
...
pointed president and chief operating ofﬁcer...”) and
ask the labeler which one of the following three
sense labels is most appropriate:
1) executive ofﬁcer of a ﬁrm, corporation, or university
...
is a speciﬁc example where non-expert annotations
...
quality in the face of worker error.
More work-
...
(1979) are the ﬁrst to consider the case of having
...
tator agreement statistics to ﬁnd bias, and use a sim-
...
examples.
(If we used maximum likelihood esti-
...
thus weighting worker votes by log
accw
1−accw . But we
use the full unconstrained multinomial model here.)
5.1.1
Example tasks: RTE-1 and event
annotation
...
examples.
Worker models were ﬁt using Laplace
...
tators. We ﬁnd a +3.4% gain on event annotation.
...
6
Training a system with non-expert
annotations
...
the classiﬁer trained with expert vs. non-expert data.
Emotion
1-Expert
10-NE
k
...
trained classiﬁers on test-set. k is the minimum number
...
average of the responses of the remaining ﬁve label-
...
pert annotations; surprisingly we ﬁnd that for ﬁve
...
beler data vs. expert annotations for ﬁve tasks found
...
fect recognition task we ﬁnd that we require an av-
erage of 4 non-expert labels per item in order to em-
ulate expert-level label quality. Finally, we demon-
strate signiﬁcant improvement by controlling for la-
beler bias.
Acknowledgments
...
ported in part by the Disruptive Technology Ofﬁce
...
In
Proc. of the Workshop on ”Word Sense Disam-
...
EM Algorithm.
Applied Statistics, Vol. 28, No. 1
(1979), pp. 20-28.
...
Baker.
2007.
Internet-Scale Collection of Human-
...
O’Hara.
1999.
Development and use of a gold-
standard data set for subjectivity classiﬁcations.
In
Proc. of ACL-1999.
