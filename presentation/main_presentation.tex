...
%------------------------------------------------------------
\begin{frame}
\frametitle{15 PCG Algorithms Compared}
...
\textbf{Result:} \alert{Partially Supported} -- ``Creative'' tag $\leftrightarrow$ wins ($r=0.42$, $p=0.0001$)
\end{block}
...
\begin{enumerate}
    ...
    \item \textbf{Mainstream} (n=7)\\Most votes, balanced play
...
Judge function correlates with human preference: \textbf{Spearman $r = 0.736$}, $p = 0.003$
\end{alertblock}
...
%------------------------------------------------------------
\begin{frame}
\frametitle{MarioDPO: Level Representation}

\begin{columns}
\column{0.6\textwidth}
\textbf{Tokenization Strategy}
\begin{itemize}
    \item \textbf{Vocabulary:} 37 ASCII characters
    \item \textbf{Direction:} \alert{Column-by-Column} (Top-to-Bottom, Left-to-Right)
\end{itemize}

\vspace{1em}
\textbf{Why Column-Major?}
\begin{itemize}
    \item Mario levels have strong \textbf{vertical dependencies} (gravity, pillars, pipes).
    \item Row-major separates vertical neighbors by the full level width ($W$).
    \item Column-major keeps vertical neighbors adjacent in the sequence.
\end{itemize}
...
\vspace{1em}
\begin{center}
Context Window: \textbf{2048 tokens} ($\approx$ 128 game columns)
\end{center}
...
%------------------------------------------------------------
\begin{frame}
\frametitle{Synthetic Data Generation (RLAIF)}

\textbf{Problem:} 571 human votes are insufficient for training a Transformer.

\vspace{0.5em}
\begin{columns}
\column{0.5\textwidth}
\textbf{Solution: Hybrid Dataset}
\begin{enumerate}
    \item \textbf{Human Data} ($10\times$ Weight)
    \begin{itemize}
        \item 473 high-confidence pairs
        \item ``Gold standard'' ground truth
    \end{itemize}
    
    \vspace{0.5em}
    \item \textbf{Synthetic Data} ($1\times$ Weight)
    \begin{itemize}
        \item Generated using the \textbf{Judge Function}
        \item 2,606 pairs generated
        \item Effectively scales training data
    \end{itemize}
\end{enumerate}

\column{0.5\textwidth}
\begin{block}{Generation Process}
\begin{enumerate}
    \item Generate 2 candidate segments ($L_A, L_B$)
    \item Evaluate with Judge:
    $$S_A = J_{final}(L_A), \quad S_B = J_{final}(L_B)$$
    \item If $|S_A - S_B| > \delta$:
    \begin{itemize}
        \item Create labeled pair $(L_{winner}, L_{loser})$
    \end{itemize}
\end{enumerate}
\end{block}
\end{columns}
...
%------------------------------------------------------------
\begin{frame}
\frametitle{DPO Training: Stage 1 -- Supervised Fine-Tuning}

\textbf{Goal:} Teach the model basic level structure before preference alignment.
...
\begin{itemize}
    \item \textbf{Architecture:} GPT-2 Small (124M params)
    \item \textbf{Vocabulary:} 37 ASCII tile characters
    \item \textbf{Context:} 2048 tokens
\end{itemize}
\end{block}

\begin{block}{SFT Training Data}
\begin{itemize}
    \item 15 Original Nintendo SMB levels
    \item Column-by-column tokenization
    \item Model learns: ground continuity, pipe structure, platform placement
\end{itemize}
\end{block}
...
%------------------------------------------------------------
\begin{frame}
\frametitle{DPO Training: Stage 2 -- Preference Alignment}

\textbf{Direct Preference Optimization (DPO)} aligns the model with human preferences without training a separate reward network.

\vspace{0.5em}
\begin{columns}
\column{0.55\textwidth}
\begin{block}{DPO Loss Function}
...
\textbf{Intuition:}
\begin{itemize}
    \item $\uparrow$ probability of \textbf{winning} levels ($y_w$)
    \item $\downarrow$ probability of \textbf{losing} levels ($y_l$)
    \item $\beta = 0.1$ controls divergence from $\pi_{ref}$
\end{itemize}

\column{0.45\textwidth}
\begin{block}{Training Config}
\begin{tabular}{ll}
Human pairs & 473 ($\times 10$) \\
Synthetic pairs & 2,606 ($\times 1$) \\
\textbf{Effective total} & \textbf{7,336} \\
\midrule
...
\begin{itemize}
    ...
    \item Validated judge function ($r = 0.74$)
...
\begin{center}
\includegraphics[width=0.70\textwidth,height=0.80\textheight,keepaspectratio]{correlation_matrix.png}
\end{center}
\end{frame}
