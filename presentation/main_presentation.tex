...
\begin{itemize}
    ...
    \item \textbf{14 generators}, \textbf{692 levels}, \textbf{428 votes} collected
\end{itemize}
...
\textbf{Result:} \alert{Not Supported} -- Linear relationship: easier $=$ better
\end{block}
...
\textbf{Result:} \alert{Strongly Supported} -- Spatial coverage predicts wins ($p=0.004$)
\end{block}
...
\textbf{Result:} \alert{Supported} -- $r = -0.324$, $p = 0.003$
\end{block}
...
\begin{itemize}
    \item 428 human pairs ($\times$10 weight)
    \item 2,637 synthetic pairs (Judge)
    \item \textbf{6,917 effective pairs}
\end{itemize}
\end{block}
...
\midrule
1 & \textbf{Original (SMB)} & 89.2\% \\
2 & \alert{MarioDPO (ours)} & -- \\
3 & ORE & 72.5\% \\
4 & Notch & 69.2\% \\
5 & MarioGPT & 62.5\% \\
\midrule
12 & patternWeightCount & 29.8\% \\
13 & patternOccur & 27.1\% \\
14 & patternCount & 22.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    Closer to Original SMB $\rightarrow$ higher win rate ($r = 0.32$)
...
\begin{itemize}
    \item Empirical comparison of 14 generators
    \item Validated judge function ($r = 0.812$)
    \item MarioDPO: SOTA among PCG methods
\end{itemize}
\end{block}

\begin{block}{Data}
\begin{itemize}
    \item 692 levels, 428 human votes
