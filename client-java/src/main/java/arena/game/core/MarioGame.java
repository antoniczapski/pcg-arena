...
public class MarioGame {
    ...
    public MarioResult playGame(String level, int timer, int marioState) {
        return this.runGame(new HumanAgent(), level, timer, marioState, true, 30, 2);
    }
    ...
    public MarioResult playGame(String level, int timer, int marioState, int fps) {
        return this.runGame(new HumanAgent(), level, timer, marioState, true, fps, 2);
    }
    ...
    public MarioResult playGame(String level, int timer, int marioState, int fps, float scale) {
        return this.runGame(new HumanAgent(), level, timer, marioState, true, fps, scale);
    }
    ...
    // ========== Phase 2: Embedded Game Support ==========

    /**
     * Callback interface for non-blocking gameplay
     */
    ...
    private GameplayCallback callback = null;
    private volatile boolean stopRequested = false;

    /**
     * Initialize render component for embedded mode (call before startGameEmbedded)
     * NOTE: Call render.init() separately AFTER adding component to visible window
     * 
     * @param scale the scale factor
     * @param agent the agent to use for input
     */
    public void initEmbeddedMode(float scale, MarioAgent agent) {
        this.render = new MarioRender(scale);
        // DON'T call render.init() here - must be called after component is added to window
        // Otherwise getGraphicsConfiguration() returns null
        this.setAgent(agent);
    }

    /**
     * Start game in embedded mode (non-blocking, no standalone window)
     * 
     * @param level the level string
     * @param timer game timer in seconds
     * @param marioState initial Mario state (0=small, 1=large, 2=fire)
     * @param callback called when game finishes
     */
    public void startGameEmbedded(String level, int timer, int marioState, GameplayCallback callback) {
        this.callback = callback;
        this.stopRequested = false;

        // Start game loop in background thread
        gameThread = new Thread(() -> {
            MarioResult result = gameLoopEmbedded(level, timer, marioState);
            if (callback != null) {
    ...
    /**
     * Get the render component for embedding
     */
    public MarioRender getRenderComponent() {
        return this.render;
    }

    /**
     * Get current world (for telemetry even if game not finished)
     */
    ...
    /**
     * Game loop for embedded mode (similar to gameLoop but assumes render is already setup)
     */
    private MarioResult gameLoopEmbedded(String level, int timer, int marioState) {
        this.world = new MarioWorld(this.killEvents);
        this.world.visuals = true; // Always use visuals in embedded mode
        this.world.initializeLevel(level, 1000 * timer);
        ...
        while (this.world.gameStatus == GameStatus.RUNNING && !stopRequested) {
            if (!this.pause) {
                //get actions
                agentTimer = new MarioTimer(MarioGame.maxTime);
                boolean[] actions = this.agent.getActions(new MarioForwardModel(this.world.clone()), agentTimer);
            ...
            //render world
            if (this.render != null && renderTarget != null) {
                this.render.renderWorld(this.world, renderTarget, backBuffer, currentBuffer);
            }
            
            //check if delay needed (30 FPS for embedded mode)
            int fps = 30;
            if (this.getDelay(fps) > 0) {
                try {
                    currentTime += this.getDelay(fps);
